...
OUTPUT_FILE = SCRIPTS_DIR / "data" / "jirimodels_verify_seller_units.csv"

DEFAULT_TARGET_STOREFRONTS = "cz"


def extract_eans_from_xml(xml_file) -> Dict[str, Dict[str, Any]]:
    """Extract EANs and their details (CODE, NAME, PRICE, STOCK) from jirimodels.xml"""
    eans_data = {}
    try:
        ...
        for product in root.findall('.//product'):
            ...
            if ean_elem is not None and ean_elem.text:
                ...
                eans_data[ean] = {
                    'code': code.text if code is not None and code.text else '',
                    'name': name.text if name is not None and name.text else '',
                    'price': price.text if price is not None and price.text else '',
                    'stock': stock.text if stock is not None and stock.text else '',
                }
...
def main():
    """Main function"""
    print(f"\n📊 Processing EANs from {XML_FILE.name}")
    ...
    # Extract EANs from XML
    eans_data = extract_eans_from_xml(XML_FILE)
